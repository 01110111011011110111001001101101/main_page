...
function createIcon(name, className = '') {
    ...
    use.setAttribute('href', `assets/icons/icons.svg?v=396799f4#icon-${name}`);
    svg.appendChild(use);
    return svg;
}
...
function copyToClipboard(text, element) {
    writeClipboard(text).then(() => {
        ...
        if (icon) {
            ...
            icon.querySelector('use')?.setAttribute('href', 'assets/icons/icons.svg?v=396799f4#icon-check');
        }
        setTimeout(() => {
            if (msg) { msg.classList.remove('opacity-100'); msg.classList.add('opacity-0'); }
            if (icon) {
                icon.dataset.icon = 'copy';
                icon.querySelector('use')?.setAttribute('href', 'assets/icons/icons.svg?v=396799f4#icon-copy');
            }
        }, 2000);
